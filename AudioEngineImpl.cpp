...
void AudioEngineImpl::initTimer()
{
    ...
    sig_event.sigev_value.sival_ptr=(void *)this;
    sig_event.sigev_notify_function = threadFunc;
    ...
    timer_val.it_interval.tv_nsec = 50000000;
    timer_val.it_value.tv_sec = 0;
    timer_val.it_value.tv_nsec = 50000000;

    timer_settime(timer_id, 0, &timer_val, nullptr);
}

void AudioEngineImpl::threadFunc(sigval_t sig)
{
    int signo = sig.sival_int;
    AudioEngineImpl *obj = (AudioEngineImpl *)sig.sival_ptr;
...
//    std::string fullPath = FileUtils::getInstance()->fullPathForFilename(path);
    std::string fullPath = QFileInfo(QString(path.c_str())).absoluteFilePath().toStdString();
...
//        std::string fullPath = FileUtils::getInstance()->fullPathForFilename(filePath);
        std::string fullPath = QFileInfo(QString(filePath.c_str())).absoluteFilePath().toStdString();
        FMOD_RESULT result = pSystem->createSound(fullPath.c_str(), FMOD_LOOP_OFF, 0, &sound);

        if (ERRCHECK(result)) {
            qDebug("sound effect in %s could not be preload\n", filePath.c_str());
            if (callback) {
                callback(false);
            }
            return -1;
