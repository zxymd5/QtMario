...
//        if ( !FileUtils::getInstance()->isFileExist(filePath)){
        if(!QFileInfo(QString(filePath.c_str())).isFile()) {
            break;
        }
        ...
        if (_audioIDInfoMap.size() >= _maxInstances) {
            qDebug("Fail to play %s cause by limited max instance of AudioEngine",filePath.c_str());
            break;
        }
        if (profileHelper)
        {
             if(profileHelper->profile.maxInstances != 0 && profileHelper->audioIDs.size() >= profileHelper->profile.maxInstances){
                 qDebug("Fail to play %s cause by limited max instance of AudioProfile",filePath.c_str());
                 break;
             }
             if (profileHelper->profile.minDelay > TIME_DELAY_PRECISION) {
                 ...
                 if (profileHelper->lastPlayTime > TIME_DELAY_PRECISION && currTime - profileHelper->lastPlayTime <= profileHelper->profile.minDelay) {
                     qDebug("Fail to play %s cause by limited minimum delay",filePath.c_str());
                     break;
                 }
             }
        }
...
bool AudioEngine::isLoop(int audioID)
{
    ...
    qDebug("AudioEngine::isLoop-->The audio instance %d is non-existent", audioID);
    return false;
}
...
float AudioEngine::getVolume(int audioID)
{
    ...
    qDebug("AudioEngine::getVolume-->The audio instance %d is non-existent", audioID);
    return 0.0f;
}
